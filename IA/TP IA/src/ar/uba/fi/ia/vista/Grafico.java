...
public class Grafico extends JPanel {
	...
	private final static BasicStroke grosor1 = new BasicStroke(5.5f); //thickness
	/**
	 ...
	 * component.
	 */
	protected List<CuadriculaGrafico> cuadriculaGraficos;
	...
	public void paint(Graphics g)
	{
	 if (entryImage == null)
		...
		for (int i = 0; i < cuadriculaGraficos.size(); i++) {
	 ...
	 * @param s
	 */
	public void setSamples(List<CuadriculaGrafico> s)
	{
		cuadriculaGraficos = s;
	}
     ...
     private int limpiarSamplesInvalidos(int nSamples){
    	 ...
    	 for (int i = 0; i < nSamples; i++) {
    		 ...
    		 bottom=downSampleBottom.get(i);
 			 ancho = (double)(rihgt-left)/(double)cuadriculaGraficos.get(0).getData().getWidth();
 			alto = (double)(bottom-top)/(double)cuadriculaGraficos.get(0).getData().getHeight();
 			if(alto*ancho<0.2){
	...
	public void downSample()
	{
		...
		try {
			...
			for (int i = 0; i < nSamples; i++) {
				...
				if (i>0){
					CuadriculaGrafico newSample = new CuadriculaGrafico(cuadriculaGraficos.get(0).getData().getWidth(),cuadriculaGraficos.get(0).getData().getHeight());
					newSample.setBounds(cuadriculaGraficos.get(0).getX(),cuadriculaGraficos.get(0).getY(),cuadriculaGraficos.get(0).getWidth(),cuadriculaGraficos.get(0).getHeight());
					cuadriculaGraficos.add(newSample);
				}

				SampleData data = cuadriculaGraficos.get(i).getData();
			...
			cuadriculaGraficos.get(0).repaint();
			repaint();
