...
public:
	...
	int actualizarIndexSet(keyType keyVieja, keyType keyNueva,bool escribirNodo,int nivel){
	...
	void bajarUnNivel(BTreeNode<keyType>* nodoPadre,BTreeNode<keyType>* nodeMergeFather){
	...
	int eliminarKeyRaiz(keyType key, BTreeNode<keyType>* nodoPadre,int nivel){
		if(nivel==0){
			nodoPadre->remove(key,-1);
			if(nodoPadre->getNumKeys()==1)
				return -2;
	...
	int eliminarIndexSet(keyType key, BTreeNode<keyType>* nodoPadre,int nivelArbol){
		...
		if((((nodoPadre->getNumKeys()-1)>=nodoPadre->getMinKeys()) && key!=nodoPadre->largestKey())){
		 ...
		 */
		else if(((nodoPadre->getNumKeys()-1)>=nodoPadre->getMinKeys()) && nodoPadre->largestKey()==key){
			nodoPadre->remove(key,nodoPadre->getRecAddrs()[nodoPadre->search(key,-1)]);
			actualizarIndexSet(key,nodoPadre->largestKey(),true,nivelArbol-1);
		...
		return 0; //retorno 0 no hago nada en el concatenar
	}
	...
	bool cumpleCondicionConcatenacion(BTreeNode <keyType>* nodo,BTreeNode <keyType>* nodoVecino, keyType key,int ordenDeComparacionNodos){
		if(ordenDeComparacionNodos==1 && nodoVecino->getIsLeaf())//verifico que el nodo pueda entrar en el nodo vecino
			return ((nodoVecino->getFreeSpace())>=(nodo->getBlockMaxSize()-(nodo->getFreeSpace()+tamanioDato(key,nodo))));
		else if(nodoVecino->getIsLeaf())
			return ((nodo->getFreeSpace()+tamanioDato(key,nodo))>=(nodoVecino->getBlockMaxSize()-nodoVecino->getFreeSpace()));
	...
	int concatenarNodos(keyType key,BTreeNode <keyType>* nodoPadre,BTreeNode <keyType>* nodo, BTreeNode <keyType>* nodoVecino, int nivelPadre){
		...
		if((key==nodoPadre->largestKey())&&(cumpleCondicionConcatenacion(nodo,nodoVecino,key,1))){
			...
			nodo->remove(key,-1);
			//int addrNodeLeaf=nodo->getRecAddr();
			actualizarIndexSet(key,nodo->largestKey(),true,nivelPadre);
			nodoPadre->updateAddr(nodo->largestKey(),nodoVecino->getRecAddr());
			keyType keyAremover = nodoVecino->largestKey();
			nodoVecino->merge(nodo);
			nodoVecino->setNextNode(nodo->getNextNode());
			freeBlocks->add(nodo->getRecAddr());
			nodeMergeFather=nodoVecino;
			ret_removeIndex=eliminarKeyRaiz(keyAremover,nodoPadre,nivelPadre);
			...
			if((key<nodoVecino->getKeys()[0])&&(cumpleCondicionConcatenacion(nodo,nodoVecino,key,0))){
				...
				nodo->remove(key,-1);
				nodoPadre->updateAddr(nodoVecino->largestKey(),nodo->getRecAddr());
				...
				ret_removeIndex=eliminarKeyRaiz(key,nodoPadre,nivelPadre);
				if(ret_removeIndex==-1)
					ret_removeIndex=eliminarIndexSet(key,nodoPadre,nivelPadre);
			}else if(key>nodoVecino->largestKey() && (cumpleCondicionConcatenacion(nodo,nodoVecino,key,1))){
				nodo->remove(key,-1);
				//int addrNodeLeaf=nodo->getRecAddr();
			...
			if((key<nodoVecino->getKeys()[0])&&(cumpleCondicionConcatenacion(nodo,nodoVecino,key,0))){
				...
				nodo->remove(key,-1);
				nodoPadre->updateAddr(nodoVecino->largestKey(),nodo->getRecAddr());
				keyType keyAremover = nodo->largestKey();
				nodo->merge(nodoVecino);
				nodo->setNextNode(nodoVecino->getNextNode());
				freeBlocks->add(nodoVecino->getRecAddr()); //al concatenar marco como libre el nodo vecino
				nodeMergeFather=nodo;
				ret_removeIndex=eliminarKeyRaiz(keyAremover,nodoPadre,nivelPadre);
				if(ret_removeIndex==-1)
					ret_removeIndex=eliminarIndexSet(keyAremover,nodoPadre,nivelPadre); //elimino porque el merge borra y inserta poniendo largestKey
			}else if(key>nodoVecino->largestKey() && (cumpleCondicionConcatenacion(nodo,nodoVecino,key,1))){
				nodo->remove(key,-1);
				nodoPadre->updateAddr(nodo->largestKey(),nodoVecino->getRecAddr());
				keyType keyAremover = nodoVecino->largestKey();
	...
	int redistribuirNodeFealLargestKey(keyType key,BTreeNode <keyType>* nodoPadre,BTreeNode <keyType>* nodo, BTreeNode <keyType>* nodoVecino,int sizeDataUnderflow){
		if(key<nodoVecino->getKeys()[0] && 	((nodoVecino->getIsLeaf() && (nodoVecino->getFreeSpace()+tamanioDato(nodoVecino->getKeys()[0],nodoVecino))<=sizeDataUnderflow)
			...
			bool isNotUnderflow=((nodoVecino->getIsLeaf() && (nodoVecino->getFreeSpace()+tamanioDato(nodoVecino->largestKey(),nodoVecino))<=sizeDataUnderflow)
	...
	void redistribuirOtroCaso(keyType key,BTreeNode <keyType>* nodoPadre,BTreeNode <keyType>* nodo, BTreeNode <keyType>* nodoVecino){
		nodo->remove(key,nodo->getRecAddrs()[nodo->search(key,-1)]);
	...
	int redistribuirClaves(keyType key,BTreeNode <keyType>* nodoPadre,BTreeNode <keyType>* nodo, BTreeNode <keyType>* nodoVecino){
		...
		if(nodoVecino->getIsLeaf())
			sizeDataUnderflow=nodoVecino->getMinFreeSpace();
		else
			sizeDataUnderflow=nodoVecino->getMinKeys();
		...
		if(key==nodoPadre->largestKey()
			...
			if(isNotUnderflowNodeSibling){ //la clave a eliminar no esta en el padre
				redistribuirOtroCaso(key,nodoPadre,nodo,nodoVecino);
			}else
		...
		if((nodo->getIsLeaf() && nodo->getFreeSpace()<=nodo->getMinFreeSpace())
				|| (!nodo->getIsLeaf() && nodo->getNumKeys()>=nodo->getMinKeys()))
	...
	int pasarDatosYclavesSinRemover(BTreeNode<keyType>* nodoOrigen, BTreeNode<keyType>* nodoDestino){
		...
		int indice;
		int sizeOfData=0;
		keyType largestKeyOldNodoDestino=nodoDestino->largestKey();
		keyType largestKeyOldNodoOrigen=nodoOrigen->largestKey();
		...
		do{
			...
			bool isUnderflowNodoOrigen=((nodoOrigen->getIsLeaf() && ((nodoOrigen->getFreeSpace()+sizeOfData)>nodoOrigen->getMinFreeSpace()))
					|| (!nodoOrigen->getIsLeaf() && ((nodoOrigen->getNumKeys()-1)<nodoOrigen->getMinKeys())));
			...
			if((nodoDestino->getIsLeaf() && nodoDestino->getFreeSpace()<=nodoDestino->getMinFreeSpace()) ||
					(!nodoDestino->getIsLeaf() && nodoDestino->getNumKeys()>=nodoDestino->getMinKeys())) //si quedo fuera de underflow no sigo en el while
				next=false;
		}while(next);

		if(largestKeyOldNodoDestino<nodoDestino->largestKey())
			actualizarIndexSet(largestKeyOldNodoDestino,nodoDestino->largestKey(),true,this->height-2);
		else if(largestKeyOldNodoOrigen>nodoOrigen->largestKey()){
			actualizarIndexSet(largestKeyOldNodoOrigen,nodoOrigen->largestKey(),true,this->height-2);
		}
	...
	int isContinueUnderflow(BTreeNode<keyType>* nodo,keyType keyRemove,BTreeNode<keyType>* nodoVecino,int sizeData){
		...
		if(nodoVecino->getIsLeaf() && (nodoVecino->caseDataRemove(sizeData)<0))
			return -2; //significa que el nodo vecino si se redistribuye esta clave quedara en underflow
		else if(!nodoVecino->getIsLeaf() && (nodoVecino->getNumKeys()-((sizeData)/(nodo->getKeySize())))<nodoVecino->getMinKeys())
			return -2; //significa que el nodo vecino si se redistribuye esta clave quedara en underflow
		...
		if(nodo->getIsLeaf()){
			if(((nodo->getFreeSpace()+tamanioDato(keyRemove,nodo))-sizeData)<=nodo->getMinFreeSpace())
				return 1; //se fue del underflow
			else
				return -1; //sigue en underflow
		}else{
			if(((sizeData/*-tamanioDato(keyRemove,nodo)*/)/(nodo->getKeySize())+nodo->getNumKeys()-1)>=nodo->getMinKeys())
				return 1; //se fue del underflow
			else
				return -1; //sigue en underflow
		}
	...
	void actualizarDireccionNodoVecino(BTreeNode<keyType>* nodoVecino,BTreeNode<keyType>* otroNodoVecino,BTreeNode<keyType>* nodo,keyType keyAcomparar){
		if(nodoVecino!=NULL && otroNodoVecino!=NULL){
			if(nodoVecino->largestKey()<keyAcomparar){
				nodoVecino->setNextNode(otroNodoVecino->getRecAddr());
				store(nodoVecino);
			}else{
...
//					delete otroVecino;
	...
	int simpleRemove(BTreeNode<keyType>*nodoHoja,keyType key,int dir,bool removeWithUnderflow,int nivelPadre){
		...
		if(nodoHoja->getNumKeys()-1==0 && removeWithUnderflow==true){
			int ret_removeIndex;
			nodoHoja->remove(key,dir);
			freeBlocks->add(nodoHoja->getRecAddr());
			store(nodoHoja);
			ret_removeIndex=eliminarKeyRaiz(key,this->nodes[nivelPadre],nivelPadre);
			if(ret_removeIndex==-1)
				ret_removeIndex=eliminarIndexSet(key,this->nodes[nivelPadre],nivelPadre);
			return 1;
		}else if(((nodoHoja->caseDataRemove(tamanioDato(key,nodoHoja))>=0 || removeWithUnderflow) && key!=nodoHoja->largestKey())||(this->height==1)){
		 ...
		 */
			...
			nodoHoja->remove(key,dir);
			actualizarIndexSet(key,nodoHoja->largestKey(),true,nivelPadre);
			this->store(nodoHoja);
			return 1;
		}
	...
	int remover(keyType key){
		...
		ret_remove=simpleRemove(nodoHoja,key,dir,removeWithUnderflow,nivelPadreHoja);
