...
int Validador::validarAperturaYCierreTags(){
	...
	while(Validador::ListaTagsArchivo.size()!=2 && (exito==-1)){
		...
		if(cadena.compare("<General>")==0){
		...
		else if(cadena.compare("<ListadoDeDispersores>")==0 || dentroListaElementos==true){
			//utilizo dentroListaElementos para trabajar entre <ListadoDeDispersores> y </ListadoDeDispersores>
			dentroListaElementos = true;
			//si la cadena es <ListadoDeDispersores> pueden pasar dos cosas, que el siguiente sea </ListadoDeDispersores> o
			...
			if(cadena.compare("</ListadoDeDispersores>")==0){

				//si encuentro <ListadoDeDispersores> seguido de </ListadoDeDispersores> los borro
				//decremento el puntero para que apunte a <ListadoDeDispersores> y iterAux2 apunta a </ListadoDeDispersores>
				iterAux1--;
				iter++;
				Validador::ListaTagsArchivo.erase(iterAux1);
				Validador::ListaTagsArchivo.erase(iterAux2);
