...
ResultadoComparacion compararDato(Dato dato1, Dato dato2) {
  ...
  if (dato1.numerocara == dato2.numerocara) return igual;     // ambos son iguales

  else
    if (dato1.numerocara > dato2.numerocara) return mayor;       // dato1 es mayo que dato2
...
void eliminarNodo(Lista &lista, PtrNodoLista ptrNodo) {
  ...
  if ((! listaVacia(lista)) && (ptrNodo != fin())) {
    ...
    /* Destruye el TDA cara */
    destruir(ptrNodo->dato.cara);
    /* Elimina el nodo */
    delete ptrNodo;
  }
}
