...
// Se define la existencia de un TDA llamado Cola
// Su objetivo es modelar el concepto de Cola, es decir, tratamiento de información
...
PRE:
	Parámetros
	void (*eliminarInfo)(void*) = NULL		--> Forma de eliminar la información contenida por la Cola

POS:
	Estado Interno
	La cola queda creada e inicializada. eliminarInfo será la función invocada al querer
	eliminar la información que quedase contenida en ella al momento de invocar su destrucción
	mediante eliminarCola.

	Valor de Retorno
	Cola*									--> Referencia a la Cola generada

	Control de Error
	El valor de retorno será null en caso de no poderse alocar la memoria necesaria
*/
Cola* nuevaCola    ( void (*eliminarInfo)(void*) = NULL );

/**
PRE:
	Parámetros
	Cola* unaCola							--> cola creada con "nuevaCola"

POS:
	Estado Interno
	La cola no puede ser utilizada nuevamente
*/
...
PRE:
	Parámetros
	Cola* unaCola							--> cola (creada con "nuevaCola") a la cual se agregará el elemento
	void* Elemento							--> referencia a almacenar en la Cola

POS:
	Estado Interno
	La referencia fue agregada a la Cola. Todo cambio que se haga a la información
    referenciada por Elemento impactará, por consiguiente en los futuros usos que se
	haga de la información guardada en la cola.
...
PRE:
	Parámetros
	Cola* unaCola							--> cola (creada por nuevaCola) a la cual se le solicita el elemento

POS:
	Estado Interno
	Sucesivas llamadas a esta primitiva vaciarán la Cola.

	Valor de Retorno
	void*									--> Referencia al último Elemento colocado con agregarElemento.

	Control de Errores
	void*									--> Será null si no quedan mas elementos en la Cola. No debe utilizarse como
												indicador de que la cola esta vacia por cuanto, se puede haber acolado un null,
...
PRE:
	Parámetros
	Cola* unaCola							--> cola (creada por nuevaCola) de la cual se quiere saber si esta vacia
