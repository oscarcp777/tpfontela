...
ResultadoComparacion compararDato(Dato dato1, Dato dato2) {
  ...
  if (dato1 -> ID_bot == dato2 -> ID_bot) return igual;     // ambos son iguales

  else
    if (dato1 -> ID_bot > dato2 -> ID_bot) return mayor;       // dato1 es mayo que dato2
...
void obtenerDatoPosicion(Lista &lista, Dato &dato, int pos) {
  ...
  /* recorre los nodos hasta llegar al último o hasta
     encontrar el nodo buscado */
  int i=1;
  while ((ptrCursor != fin()) && (!i<=pos)) {
      i++;
      ptrCursor = siguiente(lista,ptrCursor);
  }
  if ((! listaVacia(lista)) && (ptrCursor != fin()))
    dato = ptrCursor->dato;
}
