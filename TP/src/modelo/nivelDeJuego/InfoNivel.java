...
public class InfoNivel {
	...
	/** ESTE CONSTRUCTOR LO USO PARA LAS PRUEBAS **/
	public InfoNivel(Posicion posicionNaveIngreso, Posicion posicionNaveEscape, ColPooglins colPooglins, ColTerreno colTerreno, ColHabilidades habilidades){
		this.posicionNaveIngreso=posicionNaveIngreso;
		this.posicionNaveEscape=posicionNaveEscape;
		this.colPooglinsNaveIngreso = colPooglins;
		this.colTerreno = colTerreno;
		this.colHabilidades = habilidades;
		this.colPooglinsNaveEscape = new ColPooglins();
	}

	/** LEVANTA ESTA INFO DE UN XML **/
	public InfoNivel(DatosNave datosNaveIngreso, DatosNave datosNaveEscape, List listaDatosTerrenos, List listaNombresHabilidades, DatosDimensionMapa datosDimensionMapa,String nivel){
	...
	for(Iterator it=datosNaveIngreso.getPooglins().iterator();it.hasNext();){
		...
		colPooglinsNaveIngreso.agregarPoogling(poogling);
	}
	...
	private void cargarColPooglinsYposNaveEscape(DatosNave datosNaveEscape){
		...
		for(Iterator it=datosNaveEscape.getPooglins().iterator();it.hasNext();){
			...
			poogling.setMovimiento(movimiento);
			colPooglinsNaveEscape.agregarPoogling(poogling);
		}	
	}
