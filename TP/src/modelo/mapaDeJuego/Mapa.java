...
public class Mapa {
	...
	*/
	public ColPooglins obtenerPooglinsNaveIngreso(){
		return new ColPooglins(naveIngreso.obtenerPooglings().obtenerTodos());
	}
	...
	public void cambiarTerreno(Terreno terreno){
		...
		if(colTerreno.borrarTerreno(mapaTerreno.obtenerTerreno(terreno.obtenerPosicion())))
		colTerreno.agregarTerreno(terreno);
	...
	//para la persistencia lo uso, dado que los pooglins los recupero de la nave de ingreso
	public void subirPooglinsAnaveIngreso(ColPooglins pooglinsAmoverEnPlaneta){
		for(Iterator it = pooglinsAmoverEnPlaneta.iterator();it.hasNext();)
		naveIngreso.obtenerPooglings().agregarPoogling((Poogling)it.next()); //paso los pooglins que estan en el planeta a la nave de ingreso
	}
	...
	public Element toXml(Document doc){
		...
		xmlElemento.setAttribute("nivel", Integer.toHexString(nivel));
